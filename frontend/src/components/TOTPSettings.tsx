...
// Type definitions for TOTP service
...
interface TOTPBackupCodesResponse {
  backupCodes: string[];
}
...
interface TOTPService {
  ...
  verifyTOTP: (code: string, secret: string) => Promise<TOTPVerifyResponse>;
  disableTOTP: (code: string) => Promise<void>;
  generateBackupCodes: (code: string) => Promise<TOTPBackupCodesResponse>;
}

// Mock totpService - replace with actual import when service is created
...
const TOTPSettings = () => {
  ...
  // Fetch TOTP status
  const { data: totpStatus, isLoading: statusLoading } = useQuery<TOTPStatus>({
  ...
  // Setup TOTP mutation
  const setupTOTPMutation = useMutation<TOTPSetupData, Error>({
    ...
    onError: (error) => {
      toast.error(
        (error as any).response?.data?.error || "Failed to setup TOTP",
      );
    },
  });
  ...
  const disableTOTPMutation = useMutation<void, Error, string>({
  ...
  const handleVerifyTOTP = () => {
    ...
    if (!setupData?.secret) {
      toast.error("Setup data not found");
      return;
    }
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={verifyTOTPMutation.isPending || !verificationCode}
            >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4">
            <Alert variant="destructive">
              ...
              <AlertDescription>
                Disabling 2FA will make your account less secure. Consider using
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={disableTOTPMutation.isPending || !disableCode}
            >
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          <div className="space-y-4">
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                Store these codes securely. You'll need them if you lose access
            ...
            <div className="grid grid-cols-2 gap-2 p-4 bg-muted rounded-lg">
              {backupCodes.map((code, index) => (
                ...
                >
                  <span
                    className={showBackupCodes ? "" : "blur-sm select-none"}
                  >
