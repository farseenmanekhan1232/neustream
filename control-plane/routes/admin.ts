  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      while (!isUnique && attempts < 10) {
        ...
        if (
          existingInDestinations.length === 0 &&
          existingInUsers.length === 0
        ) {
          isUnique = true;
        }
        attempts++;
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const preference =
        await currencyService.getUserCurrencyPreference(userId);
      const effectiveCurrency =
        await currencyService.determineEffectiveCurrency(userId, null);
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const userId = (req as any).user.id;
      const { currency_preference } = req.body as {
