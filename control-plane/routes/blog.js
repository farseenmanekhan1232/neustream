...
router.get('/posts/:slug', async (req, res) => {
  try {
    ...
    post.structuredData = blogService.generateStructuredData(post);
...
router.get('/posts/:slug/related', async (req, res) => {
  try {
    ...
    const relatedPosts = await blogService.getRelatedPosts(post.id, parseInt(limit));
...
router.get('/tags', async (req, res) => {
  try {
    ...
    const tags = await blogService.getTags(parseInt(limit));
    res.json({ tags });
  } catch (error) {
...
router.get('/search', async (req, res) => {
  try {
    ...
    const result = await blogService.searchPosts(query.trim(), options);
    res.json({
      query: query.trim(),
      ...result
    });
...
router.get('/sitemap.xml', async (req, res) => {
  try {
    const posts = await blogService.getPosts({ limit: 1000, status: 'published' });
    const categories = await blogService.getCategories();
    ...
    xml += `
  <url>
    <loc>https://www.neustream.app/blog</loc>
    ...
    categories.forEach(category => {
      xml += `
  <url>
    <loc>https://www.neustream.app/blog/category/${category.slug}</loc>
    <lastmod>${new Date().toISOString()}</lastmod>
    ...
    posts.posts.forEach(post => {
      ...
      xml += `
  <url>
    <loc>https://www.neustream.app/blog/${post.slug}</loc>
...
router.put('/posts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (req.body.categories !== undefined) {
      await db.run('DELETE FROM blog_post_categories WHERE post_id = $1', [postId]);
...
router.post('/upload', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(501).json({
      ...
      message: 'Please implement image upload functionality with your preferred storage provider'
    });
  } catch (error) {
