...
class PaymentService {
  ...
  async createOrder(userId, planId, billingCycle = 'monthly') {
    try {
      ...
      const amount = billingCycle === 'yearly' ? planData.price_yearly * 100 : planData.price_monthly * 100; // Amount in paise
  ...
  async handleSuccessfulPayment(orderId, paymentId, signature) {
    try {
      ...
      `, [
        orderData.user_id, orderData.plan_id, orderData.billing_cycle,
        orderData.amount, orderData.currency, paymentId, orderId
      ]);

      console.log(`✅ Payment completed successfully for user ${orderData.user_id}, order ${orderId}`);
