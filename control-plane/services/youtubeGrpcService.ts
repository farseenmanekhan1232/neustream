...
import ChatConnectorService from './chatConnectorService';
...
class YouTubeGrpcService {
  ...
  private streams: Map<number, grpc.ClientReadableStream<any>>;
  private messageHistory: Map<number, Set<string>>;
  private lastMessageTimestamp: Map<number, number>;
  private youtubeProto: any;

  constructor(chatConnectorService: ChatConnectorService) {
  ...
  async startGrpcStreaming(connector: GrpcConnector): Promise<void> {
    ...
    try {
      ...
      await this.chatConnectorService.handleIncomingMessage(connector as any, {
        authorName: 'System',
        authorId: 'system',
  ...
  async getLiveChatId(config: any): Promise<string | null> {
    try {
  ...
  // Start real gRPC streaming connection
  async startRealGrpcConnection(connector: GrpcConnector, liveChatId: string, accessToken: string): Promise<void> {
    try {
      ...
      stream.on('data', (response: GrpcResponse) => {
        this.handleGrpcResponse(connector as any, response);
      });
      ...
      stream.on('error', (error: grpc.ServiceError) => {
        ...
        this.handleGrpcError(connector as any, error);
      });
  ...
  handleGrpcResponse(connector: GrpcConnector, response: GrpcResponse): void {
    try {
      if (response.items && response.items.length > 0) {
        ...
        if (messageHistory && messageHistory.size > 1000) {
        ...
        for (const message of newMessages) {
          this.processGrpcMessage(connector as any, message);
        }
      }
    } catch (error) {
  ...
  async processGrpcMessage(connector: GrpcConnector, message: GrpcMessage): Promise<void> {
    try {
      ...
      await this.chatConnectorService.handleIncomingMessage(connector as any, messageData);
    } catch (error) {
  ...
  getMessageType(grpcType?: string): string {
  ...
  async handleGrpcError(connector: GrpcConnector, error: grpc.ServiceError): Promise<void> {
  ...
  async stopGrpcStreaming(connectorId: number): Promise<void> {
...
export default YouTubeGrpcService;
